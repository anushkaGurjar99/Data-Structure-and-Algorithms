...
If first char is matched -> 
    mark the current cell BLANK so that each cell can contribute exactly 1 time. 
    check all 4 directions for 2nd character.
Then from 2nd char ->
    you will only be able to explore 3 directions, it will return false on prev direction from where it was coming
If at least one of 4 direction is matching the character -> we will continue rec in valid directions.
that's how it will make recursions.
*/
